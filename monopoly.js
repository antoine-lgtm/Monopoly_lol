...
function play() {
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "You are in jail." + (mp.getRoomId() && mp.getMyPlayer() && mp.getMyPlayer().name !== player[turn].name ? "" : "<input type='button' title='Pay $50 fine to get out of jail immediately.' value='Pay $50 fine' onclick='payfifty();' />");
...
function setup() {
	...
	if (mp.getRoomId()) {
		// En multijoueur, pas de mélange — joueur 1 = index 1, joueur 2 = index 2
		for (var i = 1; i <= pcount; i++) {
			playerArray[i - 1] = i;
		}
	} else {
...
// Informer le serveur que la partie commence avec l'état initial
if (!mp.getRoomId()) {
		play();
	} else {
		if (!mp.getRoomId()) {
